...
type SearchFormProps = {
  setProducts: Dispatch<SetStateAction<RakutenProduct[]>>;
  setLoading: Dispatch<SetStateAction<boolean>>;
};

const SearchForm: React.FC<SearchFormProps> = ({ setProducts, setLoading }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onError)} className="w-48">
      ...
      <InputError message={errors.search?.message || ""} />
    </form>
  );
};
